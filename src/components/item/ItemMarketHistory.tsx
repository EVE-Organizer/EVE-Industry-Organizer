...
export function ItemMarketHistory({
  ...
  useEffect(() => {
    let cancelled = false
    setLoading(true)
    void getMarketHistory(typeId, hub, 'all').then((result) => {
      if (cancelled) return
      setHistory(result.history)
      setLoading(false)
      onLoaded?.({ hasHistory: result.history.length > 0 })
    })
    return () => {
      cancelled = true
    }
  ...
  const summaryMetrics = summary ? (
    <dl className="item-section__metrics">
      <ItemMetric variant="inline" label="Avg price" tone="primary" value={formatIsk(summary.avgPrice)} />
      <ItemMetric
        variant="inline"
        label="Avg volume"
        hint="per day"
  ...
  const body = (
    <>
      {loading ? (
        ...
        <p className="text-sm text-base-content/50">{emptyHint ?? 'No market history for this window.'}</p>
      ) : (
        <>
          {embedded ? (
          ...
          {view === 'graph' ? (
            <>
            <div className={`item-chart-wrap min-w-0 w-full${embedded || title ? ' item-chart-wrap--compact' : ' h-56'}`}>
              <ResponsiveContainer width="100%" height="100%">
                <ComposedChart data={chartData} margin={{ top: 8, right: 8, left: 0, bottom: 0 }}>
                  ...
                  />
                  <Bar yAxisId="volume" dataKey="volume" fill="oklch(var(--p) / 0.25)" barSize={6} />
                  <Line
                    yAxisId="price"
                    type="monotone"
                    dataKey="high"
