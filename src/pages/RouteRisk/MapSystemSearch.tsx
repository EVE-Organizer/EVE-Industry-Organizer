  ...
  return (
    <div ref={rootRef} className={`plan-search-wrap ${className}`}>
      ...
      <input
        ref={inputRef}
        type="search"
        className={`plan-search-wrap__input input-xs !h-8 !min-h-8 !text-xs ${open ? 'input-primary' : ''}`}
        role="combobox"
        aria-expanded={open}
      ...
      {open && query.trim().length > 0 ? (
        <ul
          className="absolute z-30 mt-1 w-full max-h-56 overflow-y-auto rounded-lg border border-eve-border bg-base-200 shadow-lg"
          role="listbox"
        >
          {filtered.length === 0 ? (
                ...
                <li key={sys.systemId} role="option">
                  ...
                  >
                    ...
                    <span className="ml-auto flex items-center gap-2 shrink-0 opacity-60">
                      {regionName ? (
                        <span className="max-w-[5rem] truncate">{regionName}</span>
                      ) : null}
