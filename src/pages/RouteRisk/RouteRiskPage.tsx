...
import { MapSystemSearch } from '@/pages/RouteRisk/MapSystemSearch'
import { RouteRiskJumpTable, RouteRiskLegend } from '@/pages/RouteRisk/RouteRiskJumpTable'
import { RouteSummaryBar } from '@/pages/RouteRisk/RouteSummaryBar'
import { dangerBandBadgeClass, filterNotableJumps } from '@/lib/routeDanger'
import {
  findSystemByName,
  parseRouteFlag,
  routeFlagLabel,
  useGateCheckRoute,
  useMapSystemsIndex,
} from '@/pages/RouteRisk/useGateCheckRoute'
...
export function RouteRiskPage() {
  ...
  useEffect(() => {
    ...
    if (fromParam) {
      const system =
        systemsByName.get(fromParam.toLowerCase()) ?? findSystemByName(systems, fromParam)
