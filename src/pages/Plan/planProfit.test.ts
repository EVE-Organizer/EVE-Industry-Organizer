...
describe('planProfit', () => {
  ...
  it('uses scheduledWindowHours for overall job hours when provided', () => {
    const template = createDefaultPlanTemplate('test')
    template.roots = [{ id: 'root-1', productTypeId: 100, runs: 100, productionDurationHours: 10 }]
    const expandInput: ExpandPlanInput = {
      template,
    ...
    const summary = computePlanProfitSummary(
      ...
      buyPrices,
      new Map([['root-1', 10]]),
      { scheduledWindowHours: 48 },
    )

    expect(summary.jobHours).toBe(48)
    expect(summary.iph).toBeCloseTo(summary.netProfit / 48, 5)
  })
  ...
  it('does not double-count packaged self-input in setup cost', () => {
    ...
    const breakdown = computeRootSetupBreakdown(template.roots[0], selfRef, expandInput, 'Kit')

    expect(breakdown.packagedBuyCost).toBeGreaterThan(0)
    expect(row.setupCost).toBeCloseTo(breakdown.totalSetupCost, 5)
    const withoutPackaged = breakdown.totalSetupCost - breakdown.packagedBuyCost
