import { fillMissingPricesFromJita } from '@/lib/hubPriceSanity'
...
export function resolvePlanBuyUnitPrice(
  ...
  if (hubId !== PLAN_DEFAULT_BUY_HUB) {
    const jitaPrice = hubPriceMaps.get(PLAN_DEFAULT_BUY_HUB)?.get(typeId) ?? 0
    if (jitaPrice > 0) return jitaPrice
  }
...
export function mergePlanBuyPrices(
  ...
  const hubBase = hubPriceMaps.get(defaultHub) ?? new Map<number, number>()
  const jitaBase = hubPriceMaps.get(PLAN_DEFAULT_BUY_HUB) ?? new Map<number, number>()
  const merged =
    defaultHub === PLAN_DEFAULT_BUY_HUB
      ? new Map(hubBase)
      : fillMissingPricesFromJita(hubBase, jitaBase)
